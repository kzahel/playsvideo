...
import { directTmdbMetadataClient } from '../app/src/metadata/direct-tmdb.js';
import { MetadataTransportCooldownError, MetadataTransportInvalidError } from '../app/src/metadata/coordinator.js';
import { metadataRepository } from '../app/src/metadata/repository.js';
import type {
...
export function registerMetadataBridge(): void {
  chrome.runtime.onMessage.addListener((message: unknown, _sender, sendResponse) => {
    ...
    void handleMetadataMessage(message)
      .then((response) => sendResponse(response))
      .catch((error) => sendResponse(toErrorResponse(message.id, error)));

    return true;
  });
}

async function handleMetadataMessage(
...
async function handleSeriesLookup(
  ...
  return {
    id,
    message: {
      type: message.type === 'metadata:match-tv' ? 'metadata:match-tv:success' : 'metadata:get-series:success',
      entry,
    },
  };
}
...
async function handleMovieLookup(
  ...
  return {
    id,
    message: {
      type: message.type === 'metadata:match-movie' ? 'metadata:match-movie:success' : 'metadata:get-movie:success',
      entry,
    },
  };
}
...
function isMetadataRequestEnvelope(value: unknown): value is MetadataRequestEnvelope {
  return Boolean(
    value &&
      typeof value === 'object' &&
      'id' in value &&
...
function toErrorResponse(id: string, error: unknown): MetadataResponseEnvelope {
  return {
    id,
